...
**Per-example ranking (sentence-level chrF++), fine-tuned model, all task
types:** we score every fine-tuned prediction individually with
`sacrebleu.sentence_chrf` and sort, so the "best"/"worst" examples below are
selected automatically rather than by manually reading through
`eval/report.md` (see the corrected caveat in Section 7 — this replaces the
partly-manual process used in an earlier draft of this report).

*Top 5 by sentence-level chrF++ (best-performing individual outputs):*

| chrF++ | Task | Input (truncated) | Prediction (truncated) |
|---|---|---|---|
| 41.1 | explain | ततस्तत्र प्रविष्टस्य कौसल्याया निवेशनम्... | This verse conveys: And thereupon Kausalyā, having entered into her abode, became very sad, and lay down on the bed. |
| 36.1 | explain | तस्यैषा लोकनाथस्य धर्मपत्नी यशस्विनी... | This verse conveys: And that one is the daughter-in-law of the lord of the world, Sītā, who has been married to you. |
| 34.8 | explain | अथ पञ्चवटीं गच्छन्नन्तरा रघुनन्दनः... | This verse conveys: And having gone to the fifth region, Raghu's son, Rāghava, saw Bhīma in his mighty form. |
| 34.7 | explain | कल्यमुत्थाय देवानां कृत्वा पूजां यथाविधि... | This verse conveys: And having performed all the rites for the welfare of the celestials, I have offered my respects to Dasaratha, my father, as one who has been born again. |
| 33.3 | qa | गुहाश्च विविधा घोरा नानामृगगणाकुलाः... | O you of great intelligence, do you take my life and be happy with these men of diverse kinds, who dwell in forests... |

*Bottom 5 by sentence-level chrF++ (worst-performing individual outputs —
all `en2sa`, reinforcing the direction-of-translation finding above):*

| chrF++ | Task | Input (truncated) | Prediction (truncated) |
|---|---|---|---|
| 9.0 | en2sa | It began to walk at pleasure near Rāma's asylum... | तस्माद् रामेरूपं सहस्रशीघ्रमिह पुष्पका। तदा वनेष्यन्निश्च मधुराः कथयन्निश्च गच्छन्ति॥ |
| 8.9 | en2sa | And presenting him with many a charming fountain... | तस्माद् राघवं प्रियमनुज्ञानं सर्वे तदा। अपि च महातेजोऽभिषेकः कौशिकः प्रीतिं भवेत्॥ |
| 8.5 | en2sa | Some on the occasions of the Sradha (first) feast... | तेन त्वां सर्वं प्रियमनुसृत्ते ददर्श च। अभिराज्यामधीयास्तथा भवति ब्रह्मणः॥ |
| 8.4 | en2sa | Khara stationed on his car in the midst of Rākşasas... | क्षरः सहस्राक्षसाय राजा निशाचरो मुनि। कूटमुपास्ते देवी भवत् प्रभावं गच्छति॥ |
| 8.2 | en2sa | And he spent some time in serving the sacrificial fire... | तस्माद् रमपादः प्रीतिं सर्वे तेन चैव शक्तिं। अनुगच्छन्ति यदि कौशिकं गोष्टं जगत् भूतम्॥ |

Reading these side by side sharpens the corpus-level pattern from the table
above: **every one of the five worst individual outputs is `en2sa`**, and
each is fluent, well-formed Devanagari verse that bears little to no
semantic relation to its source sentence — not a partially-correct
translation, but a confident fabrication in the target script/register (see
Section 7 for discussion). The best individual outputs cluster in `explain`,
where the task only requires paraphrasing content already present in the
Sanskrit input, rather than generating novel Sanskrit — consistent with
translation direction, not task type, being the dominant factor in quality.

Full top-3/bottom-3 breakdowns per task type (`sa2en`, `en2sa`, `explain`,
`qa`, `summary`) were generated the same way and are available in the
notebook output; they show the same pattern within `sa2en`/`qa`/`summary`/
`explain` (best cases stay faithfully anchored to the source; worst cases
drift into a plausible but factually different retelling) rather than the
outright hallucination seen in `en2sa`.
...
- **Update on our evaluation process:** an earlier draft of this report chose
  "good"/"bad" examples by manually reading through `eval/report.md`, since
  `eval/evaluate.py` computed corpus-level BLEU/chrF++ and a heuristic
  failure flagger but did not rank individual predictions by sentence-level
  score. We've since added a `score_and_rank` pass (scoring every prediction
  with `sacrebleu.sentence_chrf` and sorting, run from the notebook after
  Section 6) that selects the top/bottom examples shown in Section 6
  automatically and reproducibly. The examples below were re-verified
  against that ranking rather than reproduced from the earlier manual pass.
...
- ~~A real per-example scoring/ranking pass in `eval/evaluate.py`~~ —
  **done:** a `score_and_rank` helper now scores every prediction with
  sentence-level chrF++ and sorts, automating the "best"/"worst" example
  selection used in Section 6 (see the updated caveat in Section 7). Not yet
  merged into `eval/evaluate.py` itself — currently run as a notebook cell
  after Section 6; folding it into the script proper (with a `--rank`
  flag and configurable `--top_n`/`--bottom_n`, per task type) is the
  natural next step.
