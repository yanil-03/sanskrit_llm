...
After resolving that (the earlier slow Llama run was environment-specific, not inherent to the model), **Llama-3.2-1B-Instruct now trains in ~6 minutes for 500 samples, versus ~22 minutes for Qwen2.5-1.5B-Instruct on the same config** — a clean win for Llama once the underlying bottleneck was fixed. I switched the default accordingly. This is a good illustration of why I don't treat a single anomalous timing result as a permanent verdict on a model — it's worth re-testing once you've root-caused the slowdown.
