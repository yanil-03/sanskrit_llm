...
I ended up going with Qwen2.5-1.5B-Instruct as my main model, after comparing it head-to-head against Llama-3.2-1B-Instruct and finding an 18x training-speed gap in my specific setup. Both are open, well-supported, already instruction-tuned decoder models that suit QLoRA well. Qwen not requiring gated access, plus the much faster training I actually observed, made it the more practical default. Full comparison in [09_Decisions_Comparisons.md](./09_Decisions_Comparisons.md) §9.1.
...
- **Tokenizer fragmentation:** Sanskrit costs meaningfully more tokens per character than English on both tokenizers I tested — 2.45x on Llama, 4.02x on Qwen — which shrinks how much Sanskrit content fits in context and how much gradient signal it gets per token.
...
- **Llama-3.2 being gated** cost me setup friction (license acceptance, HF token) — I ended up wiring in an ungated fallback (Qwen2.5) so I wasn't blocked on it.
- **The unexplained 18x training-speed gap** between two similarly-sized models on identical config — I narrowed it down to two likely causes (CPU offload under memory pressure, or gated-repo friction) using targeted diagnostics, but didn't fully pin down which one dominated given the time I had.
...
- Extend the tokenizer's vocabulary to handle Devanagari better — given the fragmentation I measured, this is probably the single highest-leverage next step.
